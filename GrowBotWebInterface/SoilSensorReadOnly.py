...
sensorName = 'SeeSaw Soil'
data.define_sensor(sensorName, '0x36')
...
try:
    while True:
        ...
        print("Read Sensor: " + str(sensorName) + ", Temperature: " + str(temp) + "°C,  Moisture: " + str(touch) + "%")
        # Add the sensor reading to the database.
        data.add_reading(time=reading_time, name='{0} Moisture'.format(sensorName), value=touch)
        data.add_reading(time=reading_time, name='{0} Temperature'.format(sensorName), value=temp)
